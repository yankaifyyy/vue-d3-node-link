<template>
    <svg class="node-link-diagram" :width="width" :height="height" @mousemove.capture="drag($event)" @mouseup="dragEnd($event)">
        <g class="content">
            <g class="links">
            ...
            <g class="nodes">
                <circle v-for="node,key in nodes" :key="key" :r="node.size || nodeSize" @click="nodeClick({$event, node})" @mousedown.prevent="dragStart({$event, key})" :cx="node.x" :cy="node.y" :title="node.name" :class="nodeClass(node)" :style="nodeStyle(node)"></circle>
            </g>
